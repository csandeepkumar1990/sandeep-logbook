...
export class ActivityComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    let date = new Date();
    date.setHours(info.activityTime.hour, info.activityTime.minute, 0);
    info.activityTime = date;

    if (this.activityForm.invalid) {
      return
    }
    this.glucoseService.getLatestGlucoseLevelByTime({ compareTime: info.activityTime }).subscribe((res: any) => {
      ...
      console.log('infor', info)
      this.activityLevelService.create(info)
        .subscribe((res: any) => {
